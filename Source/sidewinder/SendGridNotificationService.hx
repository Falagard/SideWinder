...
import sidewinder.INotificationService;
...
class SendGridNotificationService implements INotificationService {
	 ...
	 */
	public function new() {
		this.apiKey = Sys.getEnv("SENDGRID_API_KEY");
		this.defaultFrom = Sys.getEnv("SENDGRID_FROM_EMAIL");
		var fromName = Sys.getEnv("SENDGRID_FROM_NAME");
		this.defaultFromName = fromName != null ? fromName : "SideWinder App";

		if (this.apiKey == null || this.apiKey == "") {
			throw "SendGrid API key is required. Set SENDGRID_API_KEY.";
		}
		if (this.defaultFrom == null || this.defaultFrom == "") {
			throw "Default sender email address is required. Set SENDGRID_FROM_EMAIL.";
		}
	}
	...
	public function sendEmailAdvanced(to:String, subject:String, body:String, options:EmailOptions, ?callback:(err:Dynamic) -> Void):Void {
		try {
			...
			HybridLogger.info('Sending email via SendGrid to: $to, subject: $subject');
			...
			http.onError = function(error:String) {
				HybridLogger.error('SendGrid API error: $error');
				if (callback != null) {
					callback(error);
				}
			};
			...
			http.onStatus = function(status:Int) {
				if (status >= 200 && status < 300) {
					HybridLogger.info('Email sent successfully via SendGrid (status: $status)');
					if (callback != null) {
						callback(null);
					}
				} else {
					var errorMsg = 'SendGrid API returned status: $status';
					HybridLogger.error(errorMsg);
					if (callback != null) {
						callback(errorMsg);
					}
				}
			...
			HybridLogger.error('Exception sending email: $e');
			if (callback != null) {
				callback(e);
			}
		}
