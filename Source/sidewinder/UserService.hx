...
class UserService implements IUserService {
    ...
    public function getAll():Array<User> {
        ...
        var rs = db.requestWithParams("SELECT id, display_name, email FROM users ORDER BY id ASC", null);
        while (rs.hasNext()) {
            var r = rs.next();
    ...
    public function getById(id:Int):Null<User> {
        ...
        params.set("id", id);
        var rs = db.requestWithParams("SELECT id, display_name, email FROM users WHERE id = @id", params);
        var record = rs.next();
    ...
    public function create(user:User):User {
        ...
        conn.request(db.buildSql(insertSql, params));
    ...
    public function update(id:Int, user:User):Bool {
        ...
        conn.request(db.buildSql(updateSql, params));
    ...
    public function delete(id:Int):Bool {
        ...
        conn.request(db.buildSql(deleteSql, params));
        var rs = conn.request("SELECT changes() AS affected");
        var rec = rs.next();
        db.release(conn);
        return rec != null && rec.affected > 0;
